...
ll calcpins(int pos, int pre){
	...
	ll pinned = a[pos].second + calcpins(pos+1,pos);
	ll notpinned = abs(a[pos].first - a[pre].first) + calcpins(pos+1,pre);

	dp[pos][pre] = min(pinned,notpinned);
	return dp[pos][pre];
}
...
int main(){ 
   ...
   while(TESTS--){
   		...
   		ll ans = calcpins(1,0) + a[0].second;
